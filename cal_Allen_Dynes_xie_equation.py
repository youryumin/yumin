...
def cal_Allen_Dynes_equation():
	...
	u1 = u ** (1/3)
	print(u1)
	f_omega = 1.92 * ((total_lambda + (omega_log / omega_2) - u1) / ((math.sqrt(total_lambda)) * math.exp(omega_log / omega_2))) - 0.08
	f_u = ((6.86 * math.exp(-total_lambda / u)) / ((1/total_lambda)-u-(omega_log / omega_2))) + 1
	Tc_xie = (f_u * f_omega * omega_log / 1.2) * (
			e ** ((-1.04 * (1 + total_lambda)) / (total_lambda - u * (1 + 0.62 * total_lambda))))
	Tc_xie = round(Tc_xie, 5)
	message = {'λ:': total_lambda, 'ωlog': f'{omega_log}K', 'ω2': f'{omega_2}K', 'f1': f1, 'f2': f2, 'Tc': f'{Tc}K',
			   'Tc_with_f1f2': f'{Tc_with_f1f2}K', 'Tc_xie': f'{Tc_xie}'}
